...
class Reidentification(Metrics):

    """ Create a guess table from T and S
    Attributes are :
        :_users: M table containing all users present in the transaction data T (pandas DataFrame).
        :_ground_truth: T table containing all transaction of all user for one year (pandas DataFrame).
        :_anonymized: S table, the anonymized version of the _ground_truth (pandas DataFrame).
    ...
    def _guess_update(self, attrs):
        """ Evaluate the similtude between T and S on attributs attrs.
            Update guess consequently
        ...
        for row in self._ground_truth.itertuples():
            ...
            if value in dic_value_anon.keys():
                ...
                month = month_passed(row[self._gt_t_col['date']])
                #affect pseudo for id_user where value == value
                if self.guess[id_user][month] != "DEL" :
                    continue
                self.guess[id_user][month] = dic_value_anon[value]
        with open("guess", "w") as target:
            json.dump(self.guess, target )

    def _f_hat_constructor(self):
        """
    Construct f_hat from guess table
        """
        f_hat = pd.DataFrame(self.guess).transpose()
        f_hat = f_hat.reset_index()
        f_hat = f_hat.rename(columns={'index':'id_user'})
        return f_hat
    ...
    def s1_metric(self):
        ...
        id_item_col = self._gt_t_col['id_item']
        price_col = self._gt_t_col['price']
        self._guess_update([date_col, qty_col, price_col, id_item_col])

    def s2_metric(self):
        date_col = self._gt_t_col['date']
        qty_col = self._gt_t_col['qty']
        id_item_col = self._gt_t_col['id_item']
        self._guess_update([date_col, qty_col, id_item_col])
        date_col = self._gt_t_col['date']
        qty_col = self._gt_t_col['qty']
        price_col = self._gt_t_col['price']
        self._guess_update([date_col, qty_col, price_col])
        qty_col = self._gt_t_col['qty']
        id_item_col = self._gt_t_col['id_item']
        price_col = self._gt_t_col['price']
        self._guess_update([qty_col, id_item_col, price_col])

    def s3_metric(self):
        date_col = self._gt_t_col['date']
        qty_col = self._gt_t_col['qty']
        self._guess_update([date_col, qty_col])
        qty_col = self._gt_t_col['qty']
        id_item_col = self._gt_t_col['id_item']
        self._guess_update([qty_col, id_item_col])
        id_item_col = self._gt_t_col['id_item']
        date_col = self._gt_t_col['date']
        self._guess_update([date_col, id_item_col])
        date_col = self._gt_t_col['date']
        price_col = self._gt_t_col['price']
        self._guess_update([date_col, price_col])
        qty_col = self._gt_t_col['qty']
        price_col = self._gt_t_col['price']
        self._guess_update([qty_col, price_col])
        id_item_col = self._gt_t_col['id_item']
        price_col = self._gt_t_col['price']
        self._guess_update([id_item_col, price_col])

    def s4_metric(self):
        date_col = self._gt_t_col['date']
        self._guess_update([date_col])
        qty_col = self._gt_t_col['qty']
        self._guess_update([qty_col])
        id_item_col = self._gt_t_col['id_item']
        self._guess_update([id_item_col])
        price_col = self._gt_t_col['price']
        self._guess_update([price_col])

    def score(self):
        f_hat = self._f_hat_constructor()
        score = compare_f_files(self._f_orig, f_hat)
        # Add the score to the global score for this metric
        self._current_score.append(score)
        return score

    def barbarian_method(self):
        s1_metric()
        s2_metric()
        s3_metric()
        s4_metric()
...
class UtilityMetrics(Metrics):
    ...
    def _compare_date_gt_anon(self):
        ...
        for idx in range(self._anon_trans.shape[0]):
            ...
            tmp = abs((gt_day - anon_day).days)
            f_dest.write( str(idx) + " " +  str(self._gt_t_col['id_user']) + " " + str(tmp) + "\n")
            score += tmp
...
def main():
    ...
    AT = pd.read_csv('./data/version171.csv', sep=',', engine='c', na_filter=False, low_memory=False)
    ...
    m.s3_metric()
    m.s4_metric()
    print("S1 + S2 + S3 + S4 score : {}".format(m.score()))
    """
