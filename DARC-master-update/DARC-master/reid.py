...
class Reid(Thread):
    ...
    def __init__(self, file_to_attack, method_order, nb_element, month):
    ...
    def run(self):
        ...
        find_guess.make_guess(self.month, self.method_order, self.nb_element)
        #with lock:
        GUESS_PART.append((self.month, find_guess.guess))
...
def main():
    ...
    for i in range(NB_MONTH):
        Thread_list.append(Reid(file_to_attack, method_order, nb_element, i))
        Thread_list[i].start()
    for i in range(NB_MONTH):
        Thread_list[i].join()
    GUESS_PART.sort()
